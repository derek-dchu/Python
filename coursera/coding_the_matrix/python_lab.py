...
list_average = sum(list_of_numbers, 0.0) / len(list_of_numbers)
...
S = {-4, -2, 1, 2, 5, 0}
# Replace [ ... ] with a one-line list comprehension in which S appears
exclude_zero_list = [ (i,j,k) for i in S for j in S for k in S if i+j+k == 0 and (i,k,j) != (0,0,0) ]
...
first_of_tuples_list = [ (i,j,k) for i in S for j in S for k in S if i+j+k == 0 and (i,k,j) != (0,0,0) ][0]
...
listdict2dict = { names[id]: salary for id, salary in id2salary.items() }
